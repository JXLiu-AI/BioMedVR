...
class AttriPromptLearner(nn.Module):
    ...
    """
    def __init__(self, classnames, attribute_embeds_per_class, clip_model,
                 n_ctx=16, ctx_init=None, attr_weight=0.5):
        super().__init__()
        self.n_ctx = n_ctx
        ...
        with torch.no_grad():
            embedded = embedding(tokenized.to(embedding.weight.device)).type(dtype)  # [n_cls, 77, ctx_dim]

        self.register_buffer("token_prefix", embedded[:, :1, :])         # [n_cls, 1, ctx_dim]  SOS
        self.register_buffer("token_suffix", embedded[:, 1+n_ctx:, :])   # [n_cls, *, ctx_dim]  class+EOS
        ...
        self.register_buffer("attr_embed", attribute_embeds_per_class.mean(dim=1))  # [n_cls, ctx_dim]

    def forward(self):
        # Concatenate ctx for each class: [n_cls, n_ctx, ctx_dim]
        ctx = self.ctx.unsqueeze(0).expand(self.token_prefix.size(0), -1, -1)
        prompts = torch.cat([self.token_prefix, ctx, self.token_suffix], dim=1)  # [n_cls, 77, ctx_dim]
...
def main():
    ...
    attr_embeds = get_attr_embeds(classes, attr_files, model, num_attr=args.num_attr, device=device).float()
    print(f'  attr_embeds: {attr_embeds.shape}')

    # Build prompt learner
    prompt_learner = AttriPromptLearner(classes, attr_embeds, model, n_ctx=args.n_ctx,
                                        attr_weight=args.attr_weight).to(device).float()
