...
def biomedvr_5seed_mean(ds):
    vals = []
    for s, d, fmt in [(0,'logs_BiomedVR_all_h20_v9_new','plain'),(1,'logs_BiomedVR_all_h20_v8_new','plain')]+[(s,'logs_rebuttal_5seed','seed') for s in [2,3,4]]:
...
with open(out, 'w') as fout:
    ...
    for ds in DATASETS_11:
        ...
        ap_vals = [best_from_log(f'{ROOT}/logs_baselines_attriprompt/{ds}_seed{s}.log') for s in [1,2,3]]
        ap_vals = [v for v in ap_vals if v is not None]
        if ap_vals: fout.write(f'AttriPrompt,{ds},{statistics.mean(ap_vals):.2f}\n')
        ...
        # BLMP 3-seed mean
        blmp_vals = [best_from_log(f'{ROOT}/logs_baselines_bglm/{ds}_blmp_seed{s}.log') for s in [1,2,3]]
        blmp_vals = [v for v in blmp_vals if v is not None]
        if blmp_vals: fout.write(f'BG-LM-BLMP,{ds},{statistics.mean(blmp_vals):.2f}\n')
