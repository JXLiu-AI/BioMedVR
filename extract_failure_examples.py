...
def main():
    ...
    _, testloader, classes = build_loader(DS, DOWNSTREAM_PATH, test_tf, test_tf, batch_size=64, shot=16)
    print('Classes:', classes)

    # Load text features (re-init seed before sampling)
    set_seed(SEED)
    txt_des = clip_attr_classifier(classes, model, f'attributes/gpt3/gpt3/{DS}_des.json', num_attr=20)
    txt_dist = clip_attr_classifier(classes, model, f'attributes/gpt3/gpt3/{DS}_dist.json', num_attr=20)
    txt_conf = clip_attr_classifier(classes, model, f'attributes/gpt3/gpt3/{DS}_confuse.json', num_attr=20)
    ...
    sd = torch.load('results/fs_BioMedVR_rebuttal2/5seed_s2/16ctkidneyk3a0.5s2/best.pth', map_location=device, weights_only=False)
    ...
    with torch.no_grad():
        for x, y in testloader:
            ...
            pos = 0.5*(getLogits(20, scale, x_embs[0], txt_des, k=3) +
    ...
    for true_name, pred_name in target_pairs:
        t_idx = cls2idx[true_name]; p_idx = cls2idx[pred_name]
        ...
        for r in all_records:
            if r['true'] == t_idx and r['pred'] == p_idx:
                ...
                selected.append({
                    ...
                    'overlapping_attr': conf_strs[pred_name][0] if conf_strs.get(pred_name) else 'N/A'
                })
                break
    ...
    for s in selected:
        print(f"  true={s['true_name']:15s} pred={s['pred_name']:15s} top_neg_cls={s['top_neg_class']:15s} idx={s['global_idx']}")
        print(f"    pos[true]={s['pos_score_correct']:.3f} pos[pred]={s['pos_score_pred']:.3f}  neg[true]={s['neg_score_correct']:.3f} neg[pred]={s['neg_score_pred']:.3f}")
