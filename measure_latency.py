...
model, _ = clip.load('ViT-B/16', device=device); model.eval()
...
def bench(net, x):
    net.eval()
    with torch.no_grad():
        for _ in range(N_WARM): _ = net(x)
        torch.cuda.synchronize()
        t0 = time.time()
        for _ in range(N_ITER): _ = net(x)
        torch.cuda.synchronize()
        t1 = time.time()
...
class DualVR(nn.Module):
    def __init__(self,e):
        super().__init__()
        self.experts = nn.ModuleList([PaddingVR(224,192).to(device), PaddingVR(224,192).to(device)])
        self.gating_logits = nn.Parameter(torch.ones(2, device=device)); self.e=e
    def forward(self,x):
