...
def main():
    ...
    _, testloader, classes = build_loader(args.dataset, DOWNSTREAM_PATH, test_tf, test_tf,
                                          batch_size=64, shot=args.shot)
    ...
    acc = n_correct/total
    np.savez(args.out, confusion=confusion, classes=classes,
             probs=np.concatenate(all_probs), labels=np.concatenate(all_labels),
             gating=np.array(g), acc=acc)
