...
def main():
    ...
    p.add_argument(
        "--use_negative", type=int, default=1, help="1 to use confuse attrs, 0 to use only positive"
    )
    args = p.parse_args()
    ...
    _, testloader, classes = build_loader(
        args.dataset, DOWNSTREAM_PATH, train_tf, test_tf, batch_size=64, shot=args.shot
    )
    ...
    txt_conf = (
        ...
        if os.path.exists(conf_path) and args.use_negative
        else None
    )
