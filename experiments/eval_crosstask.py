...
def main():
    ...
    train_tf = test_tf
    _, testloader, classes = build_loader(args.tgt_dataset, DOWNSTREAM_PATH, train_tf, test_tf, batch_size=64, shot=args.shot)

    # Target text features
    txt_des = clip_attr_classifier(classes, model, f'attributes/gpt3/gpt3/{args.tgt_dataset}_des.json', num_attr=args.num_attr)
    txt_dist = clip_attr_classifier(classes, model, f'attributes/gpt3/gpt3/{args.tgt_dataset}_dist.json', num_attr=args.num_attr)
    ...
    with torch.no_grad():
        for x, y in testloader:
            ...
            total += y.size(0)
    acc = correct/total
    print(json.dumps({'src_ckpt': args.src_ckpt, 'tgt_dataset': args.tgt_dataset,
                      'acc': acc, 'n': total, 'gate_pos': g[0], 'gate_neg': g[1]}))
