...
if __name__ == "__main__":
    ...
    p.add_argument("--lr", type=float, default=40)  # the initial learning rate（(start from 1)）
    p.add_argument(
    ...
    train_process = Compose(
        [
            RandomResizedCrop(
                args.input_size, scale=(0.5, 1.0), interpolation=InterpolationMode.BICUBIC
            ),
            RandomHorizontalFlip(),
            ...
            (
                ...
                and hasattr(preprocess.transforms[-1], "__call__")
                else nn.Identity()
            ),
        ]
    )
    ...
    # Load attributes and text features
    ...
    # Keep only the single-expert VR
    ...
    def network(x):
        ...
        # weighted average of logits from des-attr and dist-attr
    ...
    optimizer = torch.optim.SGD(
        visual_reprogram.parameters(), lr=args.lr, momentum=0.9, weight_decay=1e-4
    )
