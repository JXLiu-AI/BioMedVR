...
class LearnedMaskVR(nn.Module):
    """PaddingVR variant with a learnable per-pixel sigmoid mask covering the full out_size."""
    def __init__(self, out_size, input_size, normalize=True):
        super().__init__()
        ...
        self.mask_logits = nn.Parameter(torch.zeros(out_size, out_size))  # learned mask
    ...
    def forward(self, x):
        ...
        m = torch.sigmoid(self.mask_logits).unsqueeze(0)
        x = x + torch.sigmoid(self.program) * m
        if self.norm:
            x = self.normalize(x)
        return x


class FullOverlayVR(nn.Module):
    """Apply perturbation everywhere (mask = 1 globally), no preserved center."""
    def __init__(self, out_size, input_size, normalize=True):
        super().__init__()
        from torchvision.transforms import Normalize
        self.out_size = out_size
    ...
    def forward(self, x):
        x = F.pad(x, (self.l_pad, self.r_pad, self.l_pad, self.r_pad), value=0)
        x = x + torch.sigmoid(self.program)  # mask = 1 everywhere
        if self.norm:
            x = self.normalize(x)
        return x


def make_vr_module(mask_mode, out_size, input_size):
    if mask_mode == 'border':
        return PaddingVR(out_size, input_size)
    elif mask_mode == 'full':
        return FullOverlayVR(out_size, input_size)
    elif mask_mode == 'learned':
        return LearnedMaskVR(out_size, input_size)
    else:
...
class MultiPromptVR(nn.Module):
    ...
    def __init__(self, base_size, target_size, mask_mode='border'):
        super().__init__()
        self.experts = nn.ModuleList([
            make_vr_module(mask_mode, base_size, target_size),
            make_vr_module(mask_mode, base_size, target_size),
        ])
...
class MultiPromptVR_PerClass(nn.Module):
    ...
    def __init__(self, base_size, target_size, num_classes, mask_mode='border'):
        super().__init__()
        self.experts = nn.ModuleList([
            make_vr_module(mask_mode, base_size, target_size),
            make_vr_module(mask_mode, base_size, target_size),
...
class MultiPromptVR_PerSample(nn.Module):
    """Per-sample gating: small MLP from positive image embedding -> [B, 2] gate."""
    def __init__(self, base_size, target_size, embed_dim=512, mask_mode='border'):
        super().__init__()
        self.experts = nn.ModuleList([
            make_vr_module(mask_mode, base_size, target_size),
            make_vr_module(mask_mode, base_size, target_size),
    ...
    def forward(self, x):
        outs = [expert(x) for expert in self.experts]
        return torch.stack(outs, dim=0), None  # gates computed in network() after image encoding
...
def main():
    ...
    p.add_argument('--gating_type', choices=['shared', 'per_class', 'per_sample'], default='shared')
    ...
    p.add_argument('--mask_mode', choices=['border', 'full', 'learned'], default='border')
    ...
    if args.gating_type == 'per_class':
        vr = MultiPromptVR_PerClass(out_size, args.input_size, num_classes=len(classes), mask_mode=args.mask_mode).to(device)
    elif args.gating_type == 'per_sample':
        vr = MultiPromptVR_PerSample(out_size, args.input_size, embed_dim=512, mask_mode=args.mask_mode).to(device)
    else:
        vr = MultiPromptVR(out_size, args.input_size, mask_mode=args.mask_mode).to(device)
    n_params = sum(p.numel() for p in vr.parameters())
    print(f'[CONFIG] gating={args.gating_type}, trainable_params={n_params}')

    def network(x):
        outs, gates = vr(x)  # outs: [2,B,C,H,W]; gates None for per_sample
        ...
        if args.gating_type == 'per_sample':
            gates = F.softmax(vr.gate_mlp(x_embs[0]), dim=-1)  # [B,2]
            stack = torch.stack([pos_logits, neg_logits], dim=-1)  # [B,C,2]
            fused_logits = (stack * gates.unsqueeze(1)).sum(-1)  # [B,C]
        elif args.gating_type == 'per_class':
            stack = torch.stack([pos_logits, neg_logits], dim=-1)  # [B,C,2]
            fused_logits = (stack * gates.unsqueeze(0)).sum(-1)
        else:
            stack = torch.stack([pos_logits, neg_logits], dim=0)
