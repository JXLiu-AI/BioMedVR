...
from mapping import (
    blm_reweight_matrix,
    blmp_reweight_matrix,
    label_mapping_base,
    label_mapping_calculation,
    one2one_mappnig_matrix,
)
...
def main():
    ...
    for epoch in range(args.epoch):
        ...
        if args.mapping == "ilm":
            ...
            mapping = blmp_reweight_matrix(
                visual_prompt, network, trainloader, lap=1, k=int(len(class_names) * 0.15) or 1
            )
            label_mapping = partial(label_mapping_calculation, mapping_matrix=mapping)
